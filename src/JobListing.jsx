...
export default function JobListing() {
  ...
  // Reset module-scoped output buffers on each render to avoid accumulation
  preOutput = [];
  output = null;
  ...
  if (!titles || titles.length === 0) {
    ...
    for (const run of runs) {
      // Only attempt to map when data is present and is an array.
      if (Array.isArray(run.status?.data)) {
        // Spread mapped elements into preOutput so we don't end up with nested arrays
        preOutput.push(
